...
const input: string[] = fs.readFileSync(__dirname + "/input.txt").toString().split(",");

/**
 * Part One Solution
 */

function partOneSolution(input: string[]): BigInt {
  const permutations = permutateRange(0, 4);
  let maxOutput: BigInt = BigInt(0);
...
const testInput1 = [3, 15, 3, 16, 1002, 16, 10, 16, 1, 16, 15, 15, 4, 15, 99, 0, 0].map(i => i.toString());
const testInput2 = [3, 23, 3, 24, 1002, 24, 10, 24, 1002, 23, -1, 23, 101, 5, 23, 23, 1, 24, 23, 23, 4, 23, 99, 0, 0].map(i => i.toString());
const testInput3 = [3, 31, 3, 32, 1002, 32, 10, 32, 1001, 31, -2, 31, 1007, 31, 0, 33, 1002, 33, 7, 33, 1, 33, 31, 31, 1, 32, 31, 31, 4, 31, 99, 0, 0, 0].map(i => i.toString());

assert(partOneSolution(testInput1).toString(), "43210");
assert(partOneSolution(testInput2).toString(), "54321");
assert(partOneSolution(testInput3).toString(), "65210");

const pt1Solution = partOneSolution(input);
console.log(pt1Solution.toString())
...
function partTwoSolution(input: string[]): string {
  ...
  permutations.forEach(permutation => {
    const amplifier1 = new IntCodeComputerV2(input).enqueueInput(permutation[0]);
    const amplifier2 = new IntCodeComputerV2(input).enqueueInput(permutation[1]);
    const amplifier3 = new IntCodeComputerV2(input).enqueueInput(permutation[2]);
    const amplifier4 = new IntCodeComputerV2(input).enqueueInput(permutation[3]);
    const amplifier5 = new IntCodeComputerV2(input).enqueueInput(permutation[4]);

    let finalOutput: BigInt = BigInt(0);
    ...
    while (!isHalted(amplifier1, amplifier2, amplifier3, amplifier4, amplifier5)) {
      ...
      const output2 = amplifier2.enqueueInput(BigInt(output1)).execute();
      const output3 = amplifier3.enqueueInput(BigInt(output2)).execute();
      const output4 = amplifier4.enqueueInput(BigInt(output3)).execute();
      const output5 = amplifier5.enqueueInput(BigInt(output4)).execute();
      finalOutput = BigInt(output5);
    }
...
assert(partTwoSolution(testInput4).toString(), "139629729");
assert(partTwoSolution(testInput5).toString(), "18216");

const pt2Solution = partTwoSolution(input);
console.log(pt2Solution.toString());
