...
function partTwoSolution(wire1Path, wire2Path) {
  const coords = {};
  const setter = (val, steps) => typeof val === 'undefined' ? steps : Math.min(val, steps);

  populateCoords(wire1Path, coords, 0, setter);
  populateCoords(wire2Path, coords, 1, setter);
  
  const intersections = Object.entries(coords).filter(entry => entry[1][0] && entry[1][1])
  let minSteps = Infinity;

  for(let intersection of intersections) {
    const [wire1Steps, wire2Steps] = intersection[1]
    const totalSteps = wire1Steps + wire2Steps;
...
function populateCoords(path, coords, index, setter) {
  let currX = 0, currY = 0;
  let steps = 0;

  for(let instruction of path) {
    const direction = instruction[0];
    const amt = Number(instruction.slice(1));
    for(let i = 1; i <= amt; i++) {
      steps++
      if(direction === 'U') {
        currX -= 1;
      ...
      setCoord(currX, currY, coords, index, steps, setter);
    }
  }
}

function setCoord(x, y, coords, index, steps, setter) {
  coords[`${x}:${y}`] = coords[`${x}:${y}`] || []
  coords[`${x}:${y}`][index] = setter(coords[`${x}:${y}`][index], steps)
}

// findClosestIntersection(wire1Path, wire2Path);